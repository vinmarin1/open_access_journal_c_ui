...
function sendEmailToReviewersWithDeadline()
{
    try {
        ...
        WHERE (ra.`deadline` = DATE_ADD(:currentDate, INTERVAL 3 DAY) OR ra.`deadline` = DATE_ADD(:currentDate, INTERVAL 2 DAY) OR ra.`deadline` = DATE_ADD(:currentDate, INTERVAL 1 DAY))
        ...
        if ($stmt->execute()) {
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                ...
                $daysUntilDeadline = date_diff(date_create(), date_create($deadline))->days + 1;
                ...
                if ($daysUntilDeadline >= 1 && $daysUntilDeadline <= 3) {
                    ...
                    $daysLabel = ($daysUntilDeadline == 1) ? 'day' : 'days';
                    ...
                    . '<br><br>Deadline: ' . $deadline . ' (' . $daysUntilDeadline . ' ' . $daysLabel . ' remaining).<br><br>'
