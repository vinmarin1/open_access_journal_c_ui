        ...
        if (data.success) {
          ...
          if (failedAttempts >= 3) {
            ...
            disableLoginTimer = setInterval(function () {
              ...
              $("#countDown").text(
                "Account disabled. Try again in " + countDownValue + " seconds"
              );
      ...
      $.ajax({
        ...
        success: function (response) {
          ...
          if (advancedAttempt.advanced) {
            $("#countDown").text(
              "Your Account still disabled. Try again in " +
                advancedAttempt.remainingSeconds +
                " seconds"
            );
            $("#login-button").prop("disabled", true);
            $("#login-spinner").hide();
