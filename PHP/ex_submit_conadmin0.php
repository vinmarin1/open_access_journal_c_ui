...
if (isset($_SESSION['LOGGED_IN']) && $_SESSION['LOGGED_IN'] === true) {
    ...
    $orc_idAuthor = isset($_SESSION['orc_id']) ? $_SESSION['orc_id'] : null;
}
...
if ($_SERVER['REQUEST_METHOD'] === "POST") {
    ...
    handleFileUpload($files, $contributor, $author_id, $volume, $privacy, $formattedDate, $title, $category, $abstract, $subject_areas, $keywords, $reference, $comment, $coAuthors, $primaryContacts, $firstNameC, $lastNameC, $orcidsC, $emailsC, $firstName, $lastName, $orc_idAuthor, $email, $editor, $translator);
...
function handleFileUpload($files, $contributor, $author_id, $volume, $privacy, $formattedDate, $title, $category, $abstract, $subject_areas, $keywords, $reference, $comment, $coAuthors, $primaryContacts, $firstNameC, $lastNameC, $orcidsC, $emailsC, $firstName, $lastName, $orc_idAuthor, $email
, $editor, $translator)
{
    ...
    $sql = "INSERT INTO article (`author`, `volume`, `privacy`, `date`, `title`, `journal_id`, `author_id`, `abstract`, `subject_areas`, `keyword`, `references`, `content`, `status`, `round`, `comment`, `submission_type`)
            VALUES (:author, :volume, :privacy, :date, :title, :journal_id, :author_id, :abstract, :subject_areas, :keyword, :references, :content, :status, :round, :comment, :submission_type)";
    ...
    foreach ($files as $fileKey => $fileName) {
        if (!empty($fileName)) {
            ...
            if (move_uploaded_file($_FILES[$fileKey]['tmp_name'], $uploadDirectory . $newFileName)) {
                ...
                Header("Location: ../Admin/php/journalview.php");
                exit();
            }
        }
    }
